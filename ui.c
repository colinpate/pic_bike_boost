...
void setup_ui(ui_model_t* ui_model){
    ...
    ui_model->filtered_battery_voltage = 0;
...
uint8_t update_ui(ui_model_t* ui_model){
    ...
    if (!go_to_sleep){
        uint8_t max_current = get_latest_current(ui_model);
        uint8_t target_current = min(max_current, ui_model->brightness);
...
uint8_t update_state_from_button(ui_model_t* ui_model){
    ...
    } else if (ui_model->button_press_count > 0){
        ui_model->button_press_count = 0;
